...
public class ResidentsController : ControllerBase
{
    ...
    /// <summary>
    /// Push residents to Conservice environment (mock implementation).
    /// </summary>
    [HttpPost("push")]
    public async Task<ActionResult<ResidentPushResponse>> PushToConservice([FromBody] ResidentPushRequest request)
    {
        // Validate property belongs to user
        var property = await _context.Properties
            .FirstOrDefaultAsync(p => p.PropertyId == request.PropertyId && p.UserId == _userContext.UserId);
        ...
        // Mock implementation - in production, this would call the Conservice API
        var pushId = Guid.NewGuid();

        // Log the push (could also create a DataPush record for tracking)
        Console.WriteLine($"[Conservice Push] PushId: {pushId}, PropertyId: {request.PropertyId}, ResidentCount: {residentCount}");
