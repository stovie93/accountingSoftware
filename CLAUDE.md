...
cd src/ConserviceAccounting.Api && dotnet run
...
1. Check the database connection in `src/ConserviceAccounting.Api/appsettings.json`
2. Verify PostgreSQL is running: `Host=localhost;Port=5432;Database=conservice_accounting`
3. Check API console for seeding errors on startup
4. Manually verify user exists: query `Users` table for `admin@demo.com`

**Relevant Files:**
- `src/ConserviceAccounting.Infrastructure/Data/DbSeeder.cs` - Creates/resets admin user (lines 18-46)
- `src/ConserviceAccounting.Api/Program.cs` - Runs seeding on startup (line 93)
- `src/ConserviceAccounting.Api/Controllers/AuthController.cs` - Login endpoint

## Project Structure

- **API:** `src/ConserviceAccounting.Api/` - .NET backend (port 5141)
- **Web:** `src/web/` - React frontend (Vite)
- **Core:** `src/ConserviceAccounting.Core/` - Domain entities
- **Infrastructure:** `src/ConserviceAccounting.Infrastructure/` - EF Core, data access
